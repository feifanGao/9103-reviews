...
class LocationPoint extends Point {
    ...
    public LocationPoint(double x, double y) {
        super(x, y);
        this.coordinate = "x: " + Double.toString(x) +", y: " + Double.toString(y);
    }
}
...
public class ClosestPair {
        ...
        for(int i = 0; i < points.size(); i++) {
            for(int j = 0; j < points.size(); j++) {
                if(i != j) {
                    ...
                    //If distance we found is the shortest one so far
                    if( dist < min ) {
                        min = dist;
                        pair[0] = points.get(i);
                        pair[1] = points.get(j);
    ...
    public static void main(String[] args) {
        ...
        LocationPoint test = new LocationPoint(2.33, 1.23);
        System.out.println(test.coordinate); //Allows us to print coordinate of the point easily
    }
}
